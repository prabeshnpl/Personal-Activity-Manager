...
const baseStyles = {
  ...
  code: ({ inline, children }) => 
    inline ? (
      <code className="px-2 py-1 bg-gray-100 border border-gray-200 rounded text-red-600 font-mono text-sm">{children}</code>
    ) : (
      <code className="block bg-gray-900 text-gray-100 p-3 rounded-lg font-mono text-sm overflow-x-auto whitespace-pre-wrap wrap-break-word">{children}</code>
    ),
  pre: ({ children }) => (
    <pre className="bg-gray-900 text-gray-100 p-3 rounded-lg overflow-x-auto">{children}</pre>
  ),
